...
import type { FilterCondition, FilterGroup, FilterOperator } from '~/types/automation'
...
const objectType = computed(() => props.objectType ?? '')
...
function emitGroup(patch: Partial<FilterGroup>) {
...
function addFilter() {
  ...
  emitGroup({
    conditions: [...props.group.conditions, condition]
  })
}
...
<template>
  <div>
    ...
    <USelectMenu
      ...
      value-key="value"
      class="mb-2 w-full"
      @update:model-value="emitGroup({ logic: $event as FilterGroup['logic'] })"
    />
