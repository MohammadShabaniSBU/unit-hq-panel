...
import type { DealBoardColumn, DealCard } from '~/types/deal-board'
import { dealStatusColor } from '~/composables/useDealsList'

const props = defineProps<{
  columns: Array<DealBoardColumn>
  columnLoading: Partial<Record<DealStatus, boolean>>
  pendingMoveIds?: Array<number>
}>()

const emit = defineEmits<{
  'loadMore': [status: DealStatus]
  'move': [payload: {
    cardId: number
  ...
  'update:columnCards': [status: DealStatus, cards: Array<DealCard>]
}>()
...
const sentinels = new Map<DealStatus, HTMLElement>()
const observers = new Map<DealStatus, IntersectionObserver>()
/** Edge-trigger: only load when sentinel transitions false → true */
const wasNotIntersecting = new Map<DealStatus, boolean>()

function disconnectObserver(status: DealStatus) {
  observers.get(status)?.disconnect()
  observers.delete(status)
}

function observeColumn(status: DealStatus) {
...
function setScrollRoot(status: DealStatus, el: Element | null) {
...
function setSentinel(status: DealStatus, el: Element | null) {
  if (el instanceof HTMLElement) {
...
async function onAdd(
  ...
  const fromStatus = event.item?.dataset.fromStatus as DealStatus | undefined
...
function openDeal(id: number) {
  ...
  navigateTo(`/leasing/deals/${id}`)
}

function contactInitials(card: DealCard): string {
...
function contactName(card: DealCard): string {
  return card.contact?.name?.trim() || t('pages.deals.board.untitledDeal', { id: card.id })
}
</script>
      ...
      >
        ...
        >
          <div
            ...
            @keydown.enter="openDeal(card.id)"
          >
            <div class="flex items-start gap-2.5">
              ...
              <div class="min-w-0 flex-1">
                ...
                <div class="mt-1.5 flex flex-wrap items-center gap-1.5">
                  <UBadge
                    :label="$t(`dealStatus.${card.status}`)"
                    :color="dealStatusColor(card.status)"
                    variant="subtle"
                    size="sm"
                  />
                </div>
          ...
          >
            {{ $t('pages.deals.board.emptyColumn') }}
          </p>
        </VueDraggable>
