...
import type { OfferBoardColumn, OfferCard } from '~/types/offer-board'
import { offerStatusColor } from '~/composables/useOffersList'

const props = defineProps<{
  columns: Array<OfferBoardColumn>
  columnLoading: Partial<Record<OfferStatus, boolean>>
  pendingMoveIds?: Array<number>
}>()

const emit = defineEmits<{
  'loadMore': [status: OfferStatus]
  'move': [payload: {
    cardId: number
  ...
  'update:columnCards': [status: OfferStatus, cards: Array<OfferCard>]
}>()
...
const sentinels = new Map<OfferStatus, HTMLElement>()
const observers = new Map<OfferStatus, IntersectionObserver>()
/** Edge-trigger: only load when sentinel transitions false → true */
const wasNotIntersecting = new Map<OfferStatus, boolean>()

function disconnectObserver(status: OfferStatus) {
  observers.get(status)?.disconnect()
  observers.delete(status)
}

function observeColumn(status: OfferStatus) {
...
function setScrollRoot(status: OfferStatus, el: Element | null) {
...
function setSentinel(status: OfferStatus, el: Element | null) {
  if (el instanceof HTMLElement) {
...
async function onAdd(
  ...
  const fromStatus = event.item?.dataset.fromStatus as OfferStatus | undefined
...
function contactInitials(card: OfferCard): string {
...
function contactName(card: OfferCard): string {
  return card.contact?.name?.trim() || t('pages.offers.board.untitledOffer', { id: card.id })
}
</script>
    ...
    >
      ...
      >
        <VueDraggable
          ...
          @update:model-value="(cards: Array<OfferCard>) => emit('update:columnCards', column.status, cards)"
        ...
        >
          <div
            ...
            @click="openOffer(card.id)"
            @keydown.enter="openOffer(card.id)"
          >
            <div class="flex items-start gap-2.5">
              ...
              <div class="min-w-0 flex-1">
                ...
                >
                  {{ $t('pages.offers.board.dealLabel', { id: card.deal_id }) }}
                </button>
                <div class="mt-1.5 flex flex-wrap items-center gap-1.5">
                  <UBadge
                    :label="$t(`offerStatus.${card.status}`)"
                    :color="offerStatusColor(card.status)"
                    variant="subtle"
                    size="sm"
                  />
                </div>
                ...
                >
                  {{ $t('pages.offers.board.optionsCount', { count: card.options_count }) }}
                </p>
              </div>
            </div>
          </div>
          ...
          >
            {{ $t('pages.offers.board.emptyColumn') }}
          </p>
        </VueDraggable>
