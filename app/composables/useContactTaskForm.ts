...
function createDefaultForm(): ContactTaskForm {
  return {
    title: '',
    description: '',
    type: undefined,
...
function buildPayload(form: ContactTaskForm) {
...
export function useContactTaskForm() {
  const { post } = useApi()
  const { t } = useI18n()
  const form = reactive<ContactTaskForm>(createDefaultForm())
  ...
  async function submit(contactId: number) {
