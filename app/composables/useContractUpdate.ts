...
export function useContractUpdate(id: MaybeRefOrGetter<string | number>) {
  const { patch } = useApi()
  const { t } = useI18n()
  ...
  async function updateField(field: string, value: string | null) {
    ...
    try {
      const response = await patch<ApiContractDetail>(`/api/contracts/${toValue(id)}`, payload)
      ...
      fieldErrors.value = {
        ...fieldErrors.value,
        [field]: fieldError ?? fetchError.data?.message ?? t('forms.contract.updateErrorMessage')
      }
      return null
    } finally {
