...
type SiteDetailTab = 'general' | 'floor-maps' | 'access-control' | 'service-areas' | 'phone-numbers'
...
const enabledTabs = computed(() => {
  const tabs: Array<{ key: SiteDetailTab, label: string, icon: string }> = [
    { key: 'general', label: t('settings.tabs.general'), icon: 'i-lucide-info' },
    { key: 'floor-maps', label: t('settings.tabs.floorMaps'), icon: 'i-lucide-map' },
    { key: 'service-areas', label: t('settings.tabs.serviceAreas'), icon: 'i-lucide-map-pinned' },
    { key: 'access-control', label: t('settings.tabs.accessControl'), icon: 'i-lucide-lock-keyhole' },
    { key: 'phone-numbers', label: t('settings.tabs.phoneNumbers'), icon: 'i-lucide-phone' }
  ]
...
function tabFromQuery(): SiteDetailTab | null {
  ...
  const validTabs: Array<SiteDetailTab> = ['general', 'floor-maps', 'access-control', 'service-areas', 'phone-numbers']
      ...
      <template v-else>
        ...
        <div class="mt-6">
          ...
          <FacilitySiteIntegrationsTab
            v-else-if="activeTab === 'access-control'"
            :site-id="site.id"
          />
          ...
          <FacilitySiteVoiceBridgeTab
            v-else-if="activeTab === 'phone-numbers'"
            :site-id="site.id"
          />
        </div>
      </template>
